...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            InlineKeyboardButton('🔗 🅕🅘🅛🅜 🅩🅞🅝🅔 ❷.⓿', url='https://t.me/+WFE2IOCd2fkzYWI9'),
            ],[
            InlineKeyboardButton('📣 𝔘𝔭𝔡𝔞𝔱𝔢𝔰 ℭ𝔥𝔞𝔫𝔫𝔢𝔩', url='https://t.me/+N39H_-uakq1iNTI1')
            ],[
...
async def auto_filter(client, message):
    ...
    if 2 < len(message.text) < 100:
        ...
        if imdb and imdb.get('poster'):
            await message.reply_photo(photo=imdb.get('poster'), caption=f"<b>—(••÷[ ıllıllı ɪᴍᴅʙ ᴅᴀᴛᴀ ıllıllı ]÷••)—\n\n🎞 Title: <a href={imdb['url']}>{imdb.get('title')}</a>\n📆 Year: <a href={imdb['url']}/releaseinfo>{imdb.get('year')}</a>\n🎭 Genres: {imdb.get('genres')}\n🌟 Rating: <a href={imdb['url']}/ratings>{imdb.get('rating')}</a>\n\n—(••÷[ ıllıllı ꜱᴇʀᴠᴇʀ ᴅᴀᴛᴀ ıllıllı ]÷••)—\n\n↪️ Requested:  {search}\n👤 Requested By: {message.from_user.mention}\n📤 Uploaded To: Film Zone Server\n🧑‍🔧 Get Support ✔️ Darklord", reply_markup=InlineKeyboardMarkup(btn))
        elif imdb:
